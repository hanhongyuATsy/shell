...
PNR_C=libpnr.so.3.5
PNR_A=libpnr.so.7.1
FRONT_C=libfarefront.so.5.0
FRONT_A=libfarefront.so.7.4
MNG_C=fare_cache_mng
SER_C=fare_cache_ser
MNG_A=old_price_cache_mng
SER_A=old_price_cache_ser
...
Compile()
{
	make -f makefile_pricing_linux -j8 clear
	make -f makefile_pricing_linux -j8
	
	
	make -f makefile_front_linux -j8 clear
	make -f makefile_front_linux -j8 
	
	make -f makefile_display_linux -j8 clear
	make -f makefile_display_linux -j8
}
